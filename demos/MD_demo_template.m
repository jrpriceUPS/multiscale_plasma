...
% Specifying the parameters allows for flexible testcase creation.
...
% NOTE: mean(distribution.n) should be 1/pi for the dimensionless
% parameters to be well-defined!
Gamma = 0.1;                                % default: 0.1
distribution.n  =   1/(pi*species.Nsp) * ones(species.Nsp,params.Nx);
distribution.T  =   repmat(species.charge,[1,params.Nx]).^2 .* ...
    sqrt(pi*distribution.n) / (2*Gamma);
