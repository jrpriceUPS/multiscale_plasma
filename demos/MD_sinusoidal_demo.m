%%%---------------------------------------------------------------------%%%
%
% MD demo case with a sinusoidal initialization in the particle density,
% with constant initial temperature and no initial drift velocity.
%
...
distribution.n  =   1/(pi*species.Nsp) * ones(species.Nsp,params.Nx) + ...
    A/(pi*species.Nsp)*sin(2*pi*(0:params.Nx-1)/params.Nx);
distribution.T  =   repmat(species.charge,[1,params.Nx]).^2 / (2*Gamma);
