%%%--------------------------------------------------------------------%%%
%
% Interface demo problem for the BGK function
%
...
% creates an interface with given collisional parameter and densitites for
% need mean(moments.n) to be 1/pi for dimensionless parameters to be
% consisitent with the system
% the left and right sides. (use column vectors for Gamma, n_l, n_r)
Gamma       =   0.1;
n0 = 1/pi;
n_l         =   1/(2*pi);
n_r         =   3/(2*pi);
moments.n   =   repmat(n_l,[1,params.Nx]) .* ones(species.Nsp,params.Nx);
moments.n(:,params.Nx/4:params.Nx/4+params.Nx/2-1) = ...
    repmat(n_r,[1,params.Nx/2]);
moments.T   =   repmat(species.charge,[1,params.Nx]) ./ ...
    (2*repmat(Gamma,[1,params.Nx]));
