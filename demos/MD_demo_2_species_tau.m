...
% with its own mass, charge, and initial conditions. The MD simulation is
% used to compute relaxation times tau for each species at each position.
%
...
params.Lx       =   200*pi;                 % default: 100*pi
...
params.Nsplits  =   5;                      % default: 1
params.timing   =   0;                      % default: 2
...
halfmark=round(params.Nx/2);
distribution.n(1,1:halfmark)=2*distribution.n(1,1:halfmark);
distribution.n(1,halfmark+1:params.Nx)=0.25*distribution.n(1,halfmark+1:params.Nx);
distribution.n(2,1:halfmark)=0.25*distribution.n(2,1:halfmark);
distribution.n(2,halfmark+1:params.Nx)=2*distribution.n(2,halfmark+1:params.Nx);
...
timeseries.tList = output.tList;
timeseries.HList = output.HList;

% compute tau
tau = tau_compute(species,distribution,timeseries,params);